...
import { classifyIncome, getHouseholdIncome, resolveAge, evaluateEligibility } from "./eligibility-engine.js";
...
// evaluateEligibility is re-exported above from eligibility-engine.js
// The internal stub below is kept so callsites inside this file still work.
function _evaluateEligibility(resident) {
    ...
    if (category === 'B40') {
        if (perCapita <= 500 || dependents >= 5) {
            ...
            return {
                ...
                reason: `Status: Very High Priority. Household income is ${incomeStr}, placing it in the B40 (low income) bracket. ` +
                        `Critical need confirmed: ${trigger}. ` +
                        `Qualifies for immediate welfare aid priority.`
            };
        } else if (hasOku || hasElderly) {
            return {
                status: 'Eligible', priority: 'High', color: '#ea580c', bg: '#ffedd5',
                reason: `Status: High Priority. Household income is ${incomeStr} (B40 bracket) with per capita income of ${pcStr} and ${dependents} dependant(s). ` +
                        `Vulnerability factor detected: ${vulnerability} is present in the household, elevating the priority classification.`
            };
        } else {
            return {
                status: 'Eligible', priority: 'Medium', color: '#d97706', bg: '#fef3c7',
                reason: `Status: Medium Priority. Household income is ${incomeStr} (B40 bracket) with per capita income of ${pcStr} and ${dependents} dependant(s). ` +
                        `No additional vulnerability factors detected. Eligible for standard welfare aid consideration.`
            };
        }
        ...
        if (perCapita <= 800 && dependents >= 3) {
            if (hasOku || hasElderly) {
                return {
                    status: 'Eligible', priority: 'Medium', color: '#d97706', bg: '#fef3c7',
                    reason: `Status: Medium Priority. Household income is ${incomeStr} (M40 bracket). ` +
                            `Economic strain detected: per capita income of ${pcStr} with ${dependents} dependant(s). ` +
                            `Vulnerability factor detected: ${vulnerability} is present, elevating the priority.`
                };
            }
            return {
                status: 'Eligible', priority: 'Low', color: '#65a30d', bg: '#ecfccb',
                reason: `Status: Low Priority. Household income is ${incomeStr} (M40 bracket). ` +
                        `Economic strain detected: per capita income of ${pcStr} with ${dependents} dependant(s). ` +
                        `No additional vulnerability factors detected.`
            };
        } else if (hasOku || hasElderly) {
            return {
                status: 'Eligible', priority: 'Low', color: '#65a30d', bg: '#ecfccb',
                reason: `Status: Low Priority. Household income is ${incomeStr} (M40 bracket) with per capita income of ${pcStr}. ` +
                        `Vulnerability factor detected: ${vulnerability} is present in the household.`
            };
        } else {
            return {
                status: 'Not Eligible', priority: 'None', color: '#64748b', bg: '#f1f5f9',
                reason: `Status: Not Eligible. Household income is ${incomeStr} (M40 bracket) with per capita income of ${pcStr} and ${dependents} dependant(s). ` +
                        `No eligibility conditions met under current welfare aid criteria.`
            };
        }
    } else {
        if (hasOku && hasElderly && dependents >= 5) {
            return {
                status: 'Eligible', priority: 'Low', color: '#65a30d', bg: '#ecfccb',
                reason: `Status: Low Priority. Household income is ${incomeStr} (T20 bracket). ` +
                        `Although above the standard income threshold, eligibility is granted due to the combined presence of an OKU member, an elderly member, and ${dependents} dependants.`
            };
        }
        return {
            status: 'Not Eligible', priority: 'None', color: '#64748b', bg: '#f1f5f9',
            reason: `Status: Not Eligible. Household income is ${incomeStr} (T20 bracket) with per capita income of ${pcStr}. ` +
                    `Income level is above the welfare aid eligibility threshold.`
        };
    }
}
